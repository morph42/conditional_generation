...
def sample_dm(n_sample, target=8, method='ddpm'):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    gen_net = UNetModel(image_size=32, in_channels=1, out_channels=1,
                        model_channels=64, num_res_blocks=2, channel_mult=(1, 2, 3, 4),
                        attention_resolutions=[8, 4], num_heads=4).to(device)
    gen_net.load_state_dict(torch.load('models/mnist_unet.pth'))
    gen_net.to(device)
    gen_net.eval()

    class_net = LeNet(num_classes=10).to(device)
    class_net.load_state_dict(torch.load("models/cnet.pth"))
    class_net.to(device)
    class_net.eval()
    ...
    x = torch.randn(n_sample, 1, 32, 32).to(device)
    ...
    for i in tqdm(range(epochs)):
        ...
        for _ in range(inner_steps):
            opt.zero_grad()
            probs = torch.softmax(class_net(x), dim=-1)
            fx = -torch.log(probs[:, target].clamp(max=1.0))
            loss_fn = (mu_*(x-z)).sum() + 0.5*rho * \
                ((x-z)**2).sum() + 1e2 * fx.sum()
            loss_fn.backward()
            opt.step()
